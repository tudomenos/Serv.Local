...
def index():
    if "usuario_id" in session:
        # Redirecionar para o painel admin se for admin, ou para a página principal se for utilizador normal
        if session.get("is_admin"):
            return redirect(url_for("admin_panel"))
        else:
            # Utilizador normal acede diretamente à página principal (index.html)
            return render_template("index.html", produtos=carregar_produtos_usuario(session["usuario_id"], apenas_nao_enviados=True))
    # Utilizador não autenticado vê a página de login
...
# Rota dashboard removida para evitar loops de redirecionamento
...
def admin_panel():
    ...
    return render_template("admin.html", listas_agrupadas=listas_ordenadas, termo_pesquisa=termo_pesquisa)
...
if __name__ == "__main__":
    ...
    import argparse
    parser = argparse.ArgumentParser(description='Servidor Flask para Sistema EAN')
